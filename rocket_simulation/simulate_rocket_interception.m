function min_distance = simulate_rocket_interception(rotation_time, rotation_angle, target_trajectory, initial_position, initial_time)
  ...
  min_distance = Inf;
  nearest_time = 0;
  [max_time, ~] = size(target_trajectory);
  
  while rocket_state.position(2) > -1e-6 && min_distance > 1e-6 && elapsed_time < max_time
      ...
      if elapsed_time > (rotation_time + initial_time + 1)
        distance = sqrt(sum((rocket_state.position - target_trajectory(elapsed_time,:)).^2));
        if min_distance > distance
          min_distance = distance;
          nearest_time = elapsed_time;
        end
      end
  end
  
  min_distance = min_distance * nearest_time;
end